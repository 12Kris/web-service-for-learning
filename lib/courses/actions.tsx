...
export async function getMaterialsByBlockId(
    ...
    const {data, error} = await supabase
        ...
        .select("id, block_id, title, content, material_type, order_number, flashcards(id, front, back)")
        .eq("block_id", blockId)
...
export async function updateFlashcard(cardId: number, front: string, back: string) {
    const supabase = await createClient();
    const {error} = await supabase
        .from("flashcards")
        .update({front, back})
