...
export async function createTest(
  ...
  try {
    ...
    const { data: testDataResponse, error: testError }: SupabaseResponse<Test> =
      await supabase
        .from("Test")
    ...
    for (const question of testData.questions) {
      const {
        data: questionData,
        error: questionError,
      }: SupabaseResponse<TestQuestion> = await supabase
        .from("TestQuestions")
        .insert({
          test_id: testId,
          question: question.question,
      ...
      if (questionError)
        throw new Error(
          `Error creating test question: ${questionError.message}`
        );
      ...
      const {
        ...
        error: answersError,
      }: SupabaseResponse<TestAnswer[]> = await supabase
        .from("TestAnswers")
      ...
      if (correctAnswer) {
        const correctAnswerData = insertedAnswers?.find(
          (insertedAnswer: TestAnswer) =>
            insertedAnswer?.answer === correctAnswer?.text
        );
        ...
        if (correctAnswerData) {
          ...
          if (updateError)
            throw new Error(
              `Error updating correct_id: ${updateError.message}`
            );
        }
      }
    }
...
export async function updateTest(
  ...
  try {
    ...
    if (deleteQuestionsError) {
      throw new Error(
        `Error deleting old questions: ${deleteQuestionsError.message}`
      );
    }

    const questionIds =
      testData?.questions?.map((q: Question) => Number(q.id)).filter(Boolean) ||
      [];
    ...
    if (questionIds.length > 0) {
      ...
      if (deleteAnswersError) {
        throw new Error(
          `Error deleting old answers: ${deleteAnswersError.message}`
        );
      }
    }
...
export async function createMaterial(
  ...
  if (data?.id) {
    ...
    const { error: cardError } = await supabase
      ...
      .insert(cardData);
    if (cardError)
      throw new Error(`Error creating cards: ${cardError.message}`);
  }
...
export async function updateMaterial(
  ...
  if (deleteError)
    throw new Error(`Error deleting old cards: ${deleteError.message}`);
  const cardData = materialContents.map((content: Card) => ({
    front: content.front,
    back: content.back,
  ...
  const { error: insertError } = await supabase
    ...
    .insert(cardData);
  if (insertError)
    throw new Error(`Error updating cards: ${insertError.message}`);
}
