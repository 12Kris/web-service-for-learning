...
export function CourseEditForm({ course, modules }: { course: Course; modules: Module[] }) {
  ...
  // Update the openEditModal function to fetch question answers
  const openEditModal = async (type: EditType, data: EditData) => {
    setEditType(type)

    if (type === "question" && data.id) {
      // For questions, we need to fetch the answers
      setIsLoading(true)
      try {
        ...
        if (test && test.id) {
          // Fetch the complete test data with answers
          const testData = await getTestById(test.id)
          if (testData) {
            // Find the specific question with its answers
  ...
  // Replace the handleSaveEdit function with this improved version
  const handleSaveEdit = async (updatedData: EditData) => {
    setIsLoading(true)
    try {
      if (editType === "question" && editData?.id) {
        ...
        // If we have answers data, update the answers
        if (updatedData.answers && updatedData.answers.length > 0) {
          ...
          if (test) {
            ...
            const testData = await getTestById(test.id)
            if (testData) {
              // Create updated test data with the modified question
              const updatedTestData: TestDataWithQuestion = {
                block_id: test.block_id,
                question: test.question,
                questions: testData.questions.map((q) => {
                  if (q.id === Number(editData.id)) {
                    // Replace this question's answers with the updated ones
                    return {
                      id: q.id,
              ...
              // Update the entire test with the modified question and answers
      ...
      await fetchBlockData() // Refresh the data
      setEditModalOpen(false)
    } catch (error) {
      console.error("Error updating:", error)
    } finally {
