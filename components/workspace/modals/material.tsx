...
export function MaterialModal({
  ...
  const [initialCards, setInitialCards] = useState<Card[]>([]); // Старі картки
  const [newCards, setNewCards] = useState<Card[]>([]); // Нові картки, додані користувачем

  // Завантажуємо старі картки при відкритті модалки, якщо є currentMaterial
  useEffect(() => {
    const loadCards = async () => {
      if (currentMaterial?.id) {
        const cards = await getCardsByLearningMaterial(currentMaterial.id);
        ...
        setNewCards([]); // Очищаємо нові картки при завантаженні
      } else {
        setInitialCards([]);
        setNewCards([]);
      }
  ...
  // Встановлюємо заголовок матеріалу
  useEffect(() => {
    if (currentMaterial) {
      setMaterialTitle(currentMaterial.title || "");
    } else {
