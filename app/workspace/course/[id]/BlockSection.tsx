...
import { createTest, deleteTest, updateTest, createMaterial, deleteMaterial, updateMaterial } from "@/lib/tests/actions";
  ...
  const handleSaveMaterial = async (blockId, materialData, materialContents) => {
    ...
    try {
      if (currentMaterial?.id) {
        ...
        console.log({ title: materialData.title, block_id: blockId }, materialContents)
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 mb-4">
      ...
      <div className="flex justify-between">
        ...
        <Button
          variant="destructive"
          onClick={() => setModals((prev) => ({ ...prev, deleteBlock: block.id }))}
        >
          Delete Block
        </Button>
      </div>
